...
func _create_area(collision_size: float, collision_pos: float, pos: float) -> Area2D:
	...
	collision.shape.size.y = collision_size
...
func _create_top_obstacle() -> void:
	...
	var obstacle: Area2D = _create_area(shape_size, shape_size / 2.0, 0.0)
...
func _create_bottom_obstacle() -> void:
	var vertical_size: float = get_viewport_rect().size.y
	var shape_size: float = clamp(offset - SPACE, 0.0, vertical_size)
	var obstacle: Area2D = _create_area(shape_size, -shape_size / 2.0, vertical_size)
...
func _create_point_area() -> void:
	...
	var area: Area2D = _create_area(SPACE * 2.0, SPACE, area_pos)
	var collision: CollisionShape2D = area.get_node("CollisionShape2D")
	collision.shape.size.x = 20
	area.name = "PointArea"
	area.body_entered.connect(_on_point_area_body_entered)
	add_child(area, true)
